...
in a Vite + React project when more flexible loading strategies are required.
...
This page continues from the [previous chapter](../vite-react), reusing the same messages structure and overall setup.  
In this example, we switch to dynamic loading, so static `messages` are removed from the configuration.
