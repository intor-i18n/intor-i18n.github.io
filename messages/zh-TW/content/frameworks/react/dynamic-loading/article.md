...
本頁示範在 React 專案中，當需要更彈性的載入策略時，
如何使用 Intor 動態載入翻譯內容。

---

> 若希望減少初次 bundle 體積，或需要依語系動態取得最新的翻譯內容，  
> 則可改用動態載入，將 `messages` 的載入時機延後至實際使用時。

本頁延續 [前一章](../react) 的範例，沿用相同的 messages 結構與設定方式。  
...
此方式透過 **HTTP 請求** 從伺服器載入翻譯內容，在語系變更時動態取得對應的 `messages`。
...
透過自訂 `loader` 以 fetch 方式載入遠端翻譯內容，並集中於一個獨立的 `I18nProvider` 中管理。  
`useIntor` 負責整合語系狀態與載入邏輯，並將結果交由 `IntorProvider`。
